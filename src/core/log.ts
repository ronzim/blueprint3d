 ...
 */
export function isLogging(context: ELogContext, level: ELogLevel): boolean {
 ...
 */
export function log(context: ELogContext, level: ELogLevel, message: string): void {
...
if (typeof globalThis !== 'undefined' && (globalThis as any).BP3D) {
  (globalThis as any).BP3D.Core.ELogContext = ELogContext;
  (globalThis as any).BP3D.Core.ELogLevel = ELogLevel;
  (globalThis as any).BP3D.Core.logContext = logContext;
  (globalThis as any).BP3D.Core.isLogging = isLogging;
  (globalThis as any).BP3D.Core.log = log;
}
